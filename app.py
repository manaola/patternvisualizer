...
from PIL import Image, ImageEnhance, ImageDraw, ImageFont, ImageOps, ImageColor
import io
...
if pattern_file and fabric_hex and print_hex:
    ...
    # Convert print color to RGB tuple
    print_rgb = ImageColor.getrgb(print_hex)

    # Convert pattern to grayscale and tint with print color
    gray_pattern = ImageOps.grayscale(pattern)
    colored_pattern = ImageOps.colorize(gray_pattern, black="black", white=print_hex).convert("RGBA")

    # Composite over fabric background
    final_image = Image.alpha_composite(bg, colored_pattern)
